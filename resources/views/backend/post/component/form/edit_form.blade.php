<form id="myForm" class="p-4 pt-3" action="{{ route('admin.post.update', $post->id) }}" method="post" enctype="multipart/form-data">
    @csrf
    @method('put')
    <div class="form-group">
        <div class="row">
                            ...
                            <div class="card-body pt-0">
                                <div class="d-grid">
                                    <div class="row mb15">
                                        ...
                                        <div class="col-lg-6 col-6">
                                            <img id="imagePreview" src="{{ session('image_temp') ? Storage::url(session('image_temp')) : checkMinioImage($post->image) }}" alt="Image Preview" style="display: {{ session('image_temp') || checkMinioImage($post->image) ? 'block' : 'none' }}; max-width: 100px; margin-left: 10px;">
                                        </div>
            ...
            <div class="col-lg-8 col-12 mb-2 mb-lg-1">
                <label for="postTitle" class="form-label @error('title') is-invalid @enderror">Post Title</label>
                <input type="text" class="form-control" id="postTitle" name="title" value="{{ old('title', $post->title ?? "No data available") }}" aria-describedby="emailHelp" placeholder="Enter post title">
                @error('title')
    ...
    <div class="form-group">
        <div class="row">
            ...
            <div class="col-lg-12 col-12 mb-2 mb-lg-1">
                ...
                <textarea name="content" class="form-control @error('status') is-invalid @enderror" placeholder="" autocomplete="off" id="editor" data-height="500" {{ isset($disabled) ? 'disabled' : '' }}>{!! old('content', $post->content ?? 'No data available') !!}</textarea>
                @error('content')
                <div class="invalid-feedback">{{ $message }}</div>
                @enderror
            </div>
